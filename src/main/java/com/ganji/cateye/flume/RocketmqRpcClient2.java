...
public class RocketmqRpcClient2 extends AbstractRpcClient {
	private static final Logger LOGGER = LoggerFactory.getLogger(RocketmqRpcClient2.class);
	...
	public RocketmqRpcClient2() {
		...
		callTimeoutPool = Executors.newCachedThreadPool(new ThreadFactory() {
			...
			public Thread newThread(Runnable r) {
				...
				t.setName("RocketmqRpcClient RPC thread - " + String.valueOf(threadCounter.incrementAndGet()));
				LOGGER.warn("RocketmqRpcClient: add new thread. name=" + t.getName());
				return t;
			}
		});
	}
	...
	public void appendBatch(final List<Event> events) throws EventDeliveryException {
		...
		Future<Void> future = callTimeoutPool.submit(new Callable<Void>() {
			@Override
	...
	private void doAppendBatch(final List<Event> events) throws Exception {
